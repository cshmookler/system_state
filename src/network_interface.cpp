...
syst::optional_t<std::list<network_interface_t>> network_interface_t::all() {
...
syst::optional_t<bool> network_interface_t::physical() const {
    fs::path real_path;
    try {
        real_path = fs::read_symlink(this->sysfs_path_);
    } catch (const fs::filesystem_error&) {
        return SYST_NEW_ERROR("Failed to read the symlink at a path "
...
syst::optional_t<bool> network_interface_t::loopback() const {
...
syst::optional_t<network_interface_t::status_t> network_interface_t::status()
  const {
    ...
    return SYST_NEW_ERROR("An invalid status was read from a network interface "
                          "status file.\n\tstatus: '"
      + status.value() + "'\n\tfile: '" + status_path.string() + "'");
}

syst::optional_t<network_interface_t::stat_t> network_interface_t::stat()
  const {
